...
function findUnknownPredicates(F::EdgeMarkedFlag, fixed::Vector{U}) where {U<:AbstractVector{Int}}
...
function permute(F::EdgeMarkedFlag{T}, p::AbstractVector{Int}) where {T<:Flag}
    EdgeMarkedFlag{T}(permute(F.F, p), Set(permute(e, p) for e in F.marked))
end
...
function isolatedVertices(F::EdgeMarkedFlag)
    return BitVector([false for i = 1:size(F)])
end

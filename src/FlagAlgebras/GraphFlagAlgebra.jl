...
function permute(pred::EdgePredicate, p::AbstractVector{Int})
    EdgePredicate(p[pred.i], p[pred.j])
end

function findUnknownPredicates(F::Graph, fixed::Vector{U})::Vector{Vector{EdgePredicate}} where {U<:AbstractVector{Int}}
    res = EdgePredicate[]
...
function isBipartite(F::Graph)
    ...
    while any(notDone)
        ...
        if i === nothing
            ...
            neighbors = [j for j = 1:n if i != j && F.A[i,j]]

...
isAllowed(A::ProductFlag) = all(x->isAllowed(x), A.Fs)
...
function findUnknownPredicates(
    F::ProductFlag{FT}, fixed::Vector{U}, predLimits::Vector{Int}
    ...
    for (i, Ft) in enumerate(fieldtypes(FT))
        ...
        FIP = findUnknownPredicates(F.Fs[i])
        @assert length(FIP) == 1
        for p in FIP[1]
            push!(tmp, (i, p))
        end
...
function countEdges(F::ProductFlag)::Vector{Int}
    return vcat([countEdges(f) for f in F.Fs]...)
end
